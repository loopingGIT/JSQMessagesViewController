    ...
    if ([messageItem isMediaMessage]) {
        ...
        for (UIView *subview in lastPressView.subviews) {
            if ([subview respondsToSelector:@selector(setHighlighted:)]) {
                [subview performSelector:@selector(setHighlighted:) withObject:nil];
            }
        ...
        if (lastPressSubview != NULL) {
            if ([lastPressSubview respondsToSelector:@selector(setHighlighted:)]) {
                NSMethodSignature* signature = [[lastPressSubview class] instanceMethodSignatureForSelector: @selector( setHighlighted: )];
                NSInvocation* invocation = [NSInvocation invocationWithMethodSignature: signature];
                [invocation setTarget: lastPressSubview];
...
{
    CGFloat contentHeight = self.inputToolbar.contentView.textView.contentSize.height + 16;
    
    CGFloat proposedHeight = contentHeight;//self.toolbarHeightConstraint.constant + dy;
